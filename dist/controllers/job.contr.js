...
class FileDataController {
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      try {
        ...
        if (!updatedFileData) {
          return res
            .status(404)
            .json({ message: "Job topilmadi", status: 404 });
        }
        const updatedFields = {
        ...
        updatedFileData = yield FileDataModel.findByIdAndUpdate(
          fileId,
          updatedFields,
          { new: true }
    ...
    return __awaiter(this, void 0, void 0, function* () {
      try {
        ...
        if (jobskills) {
          ...
          let jobskillupdate = yield FileDataModel.findByIdAndUpdate(
            newFileData,
            {
              jobSkills: jobSkills._id,
            }
          );
          yield jobskillupdate === null || jobskillupdate === void 0
            ? void 0
            : jobskillupdate.save();
        }
        // add pull add
        if (typeMoney) {
          ...
          let jobmoneyTypeupdate = yield FileDataModel.findByIdAndUpdate(
            newFileData,
            {
              moneyTypeId: moneyType._id,
            }
          );
          yield jobmoneyTypeupdate === null || jobmoneyTypeupdate === void 0
            ? void 0
            : jobmoneyTypeupdate.save();
        }
        // add more info;
        if (moreInfo) {
          ...
          let jobInfoupdate = yield FileDataModel.findByIdAndUpdate(
            newFileData,
            {
              $push: {
          ...
          yield jobInfoupdate === null || jobInfoupdate === void 0
            ? void 0
            : jobInfoupdate.save();
        }
        //  save user;
  ...
  getAllFileData(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
      try {
        ...
        if (token) {
          let userId = JWT.VERIFY(token).id;
          const oneRecruiterJobs = yield Recruiter.findById(userId).populate(
            "posts"
          );
          return res.status(200).json(oneRecruiterJobs);
        } else {
          const fileDataList = yield FileDataModel.find().populate(
            "jobSkills jobEmployee moneyTypeId catId"
          );
  ...
  getFileDataById(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
      const fileId = req.params.id;
      try {
        const fileData = yield FileDataModel.findById(fileId).populate(
          "jobSkills jobEmployee moreInfo moneyTypeId catId employees"
        );
        if (!fileData) {
          return res
            .status(404)
            .json({ message: "FileData topilmadi", status: 404 });
        }
  ...
  searchByCriteria(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
      try {
        const _a = req.query,
          { comLocation, comName, jobTitle } = _a,
          restQuery = __rest(_a, ["comLocation", "comName", "jobTitle"]);
        let query = {}; // Empty object to hold the search criteria
        // Check if comLocation is provided and not equal to "all"
        if (
          comLocation &&
          typeof comLocation === "string" &&
          comLocation.toLowerCase() !== "all"
        ) {
        ...
        const fileData = yield FileDataModel.find(query, "-__v").populate(
          "jobSkills jobEmployee moneyTypeId catId"
        );
        res.json(fileData);
      } catch (error) {
